...
namespace Uploadarr.Common
{
    ...
    public class ServiceProvider : IServiceProvider
    {
        ...
        {
            Log.Debug("Checking if service {0} exists.", name);
            return
        ...
        {
            Log.Debug("Checking if '{0}' service is running", name);
        ...
        {
            Log.Info("Stopping {0} Service...", serviceName);
            var service = GetService(serviceName);
            if (service == null)
            {
                Log.Warn("Unable to stop {0}. no service with that name exists.", serviceName);
                return;
            }

            Log.Info("Service is currently {0}", service.Status);
            ...
            if (service.Status != ServiceControllerStatus.Stopped)
            {
                ...
                if (service.Status == ServiceControllerStatus.Stopped)
                {
                    Log.Info("{0} has stopped successfully.", serviceName);
                }
                else
                {
                    Log.Error("Service stop request has timed out. {0}", service.Status);
                }
            }
            else
            {
                Log.Warn("Service {0} is already in stopped state.", service.ServiceName);
            }
        }
        ...
        public void Start(string serviceName)
        {
            ...
            if (service == null)
            {
                Log.Warn("Unable to start '{0}' no service with that name exists.", serviceName);
                return;
            }

            if (service.Status != ServiceControllerStatus.Paused && service.Status != ServiceControllerStatus.Stopped)
            {
                Log.Warn("Service is in a state that can't be started. Current status: {0}", service.Status);
            }
            ...
            {
                Log.Info("{0} has started successfully.", serviceName);
            }
            else
            {
                Log.Error("Service start request has timed out. {0}", service.Status);
            }
        }
